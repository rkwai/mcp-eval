...
type MockState = {
  ...
  activityStore: Map<string, LoyaltyActivity[]>;
  rewards: Reward[];
  offers: Offer[];
...
function lookupCustomerById(id: string) {
  ...
  return {
    ...customer,
    recentActivity: state.activityStore.get(id)?.slice(0, 5) ?? [],
  };
}
...
function pushActivity(entry: Omit<LoyaltyActivity, 'id' | 'occurredAt'> & { occurredAt?: string }) {
  ...
  const list = state.activityStore.get(entry.customerId) ?? [];
  list.unshift(activity);
  state.activityStore.set(entry.customerId, list);
  return activity;
}
...
function assignOfferToCustomer(customerId: string, offerId: string, expiresAt?: string) {
  ...
  const list = state.customerOfferStore.get(customerId) ?? [];
  list.push(entry);
  state.customerOfferStore.set(customerId, list);
...
function claimOfferForCustomer(customerId: string, customerOfferId: string): ClaimOfferResponse {
  ...
  const offer = state.offers.find((item) => item.id === entry.offerId);
  if (!offer) {
  ...
  const reward = state.rewards.find((item) => item.id === offer.rewardId);
  if (!reward) {
